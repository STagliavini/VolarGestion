...
if (count($pilotos)) {
    foreach ($pilotos as $piloto) {
        echo '<tr>
                ...
                <td>' . $piloto->getMail_piloto().'</td>
                <td>' . $piloto->getDeuda_piloto().'</td>
                <td><form method="POST"><input type="hidden" name="id_piloto" value="' . $piloto->getId_piloto() . '"/><Button class="btn-sm btn-primary" type="submit" name="modificar" id="modificar" onclick="return verificar(' . $piloto->getId_piloto() . ');"><i class="fas fa-edit"></i></Button><Button class="btn-sm btn-danger" type="submit" name="borrar" id="borrar" onclick="return eliminar(' . $piloto->getId_piloto() . ');"><i class="fa fa-times"></i></Button></form></td>
             </tr>';
    }
}
